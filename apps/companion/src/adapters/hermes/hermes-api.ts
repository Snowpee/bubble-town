...
import { getSessionDetail, getSessionIdForResponse, getSessionSummary } from '../../store/session-store.js';
...
function buildHermesRequestHeaders(context: ChatTurnContext): HeadersInit {
  ...
  const apiKey = context.apiKey || process.env.HERMES_API_KEY || process.env.API_SERVER_KEY || process.env.BUBBLE_TOWN_HERMES_API_KEY;
...
function createStatelessChatTurnContext(
  profileId: string,
  executionOptions: ChatExecutionOptions = {},
  runtimeOverride?: Partial<Pick<ChatTurnContext, 'apiBaseUrl' | 'apiKey' | 'managedGatewayProfile' | 'model' | 'systemPrompt'>>,
): ChatTurnContext {
  const managedGatewayProfile = runtimeOverride?.managedGatewayProfile
    ?? (
      executionOptions.managedGatewayProfileId === profileId
      || isManagedHermesGatewayProfile(profileId)
    );
  const runtime = resolveProfileRuntime(profileId, managedGatewayProfile);

  return {
    profileId,
    apiBaseUrl: runtimeOverride?.apiBaseUrl ?? executionOptions.apiBaseUrl ?? getHermesApiBaseUrl(),
    managedGatewayProfile: runtimeOverride?.managedGatewayProfile ?? managedGatewayProfile,
    model: runtimeOverride?.model ?? runtime.model,
    systemPrompt: runtimeOverride?.systemPrompt ?? runtime.systemPrompt,
...
function isDeepSeekAuxiliaryRuntime(runtime: ReturnType<typeof resolveAuxiliaryLlmRuntime> | undefined): boolean {
  if (!runtime) {
    return false;
  }
  return /deepseek/i.test(runtime.baseUrl) || /deepseek/i.test(runtime.model);
}

function applyAuxiliaryProviderOptions(
  payload: JsonRecord,
  runtime: ReturnType<typeof resolveAuxiliaryLlmRuntime> | undefined,
): JsonRecord {
  if (!isDeepSeekAuxiliaryRuntime(runtime)) {
    return payload;
  }

  const thinkingEnabled = runtime?.thinkingEnabled === true;
  ...
  return {
    ...
    ...(thinkingEnabled
      ? {
          reasoning_effort: runtime?.reasoningEffort ?? 'high',
        }
      : {}),
  };
}

function buildStructuredChatCompletionsPayload(
  context: ChatTurnContext,
  request: StructuredDataRequest,
): JsonRecord {
  const instructions = [context.systemPrompt, request.runtimeInstructions].filter(Boolean).join('\n\n');
  ...
  return {
    ...
    }, request.taskType ? resolveAuxiliaryLlmRuntime(request.profileId || DEFAULT_PROFILE_ID, request.taskType) : undefined),
  };
}

function buildStructuredPromptOnlyInstructions(
  context: ChatTurnContext,
  request: StructuredDataRequest,
): string {
  return [
    ...
    `输出必须满足以下 JSON Schema: ${JSON.stringify(request.schema)}`,
  ].filter(Boolean).join('\n\n');
}

function buildStructuredChatCompletionsPromptOnlyPayload(
  context: ChatTurnContext,
  request: StructuredDataRequest,
): JsonRecord {
  return {
    ...applyAuxiliaryProviderOptions({
      ...
      messages: [
        {
          role: 'system',
          content: buildStructuredPromptOnlyInstructions(context, request),
        },
        {
          role: 'user',
          content: request.input,
...
function buildStructuredChatCompletionsJsonObjectPayload(
  context: ChatTurnContext,
  request: StructuredDataRequest,
): JsonRecord {
  return {
...
function isUnsupportedStructuredResponseFormat(status: number, details: string): boolean {
  return status === 400 && details.includes('This response_format type is unavailable now');
}
...
export async function requestStructuredData<TResult>(
  ...
  const profileId = request.profileId || DEFAULT_PROFILE_ID;
  const auxiliaryRuntime = request.taskType ? resolveAuxiliaryLlmRuntime(profileId, request.taskType) : undefined;
  const context = createStatelessChatTurnContext(
    profileId,
    executionOptions,
    auxiliaryRuntime
      ? {
          apiBaseUrl: auxiliaryRuntime.baseUrl,
          apiKey: auxiliaryRuntime.apiKey,
          managedGatewayProfile: false,
          model: auxiliaryRuntime.model,
        }
      : undefined,
  );
  const controller = auxiliaryRuntime ? new AbortController() : undefined;
  const timeout = auxiliaryRuntime ? setTimeout(() => controller?.abort(), auxiliaryRuntime.defaultTimeoutMs) : undefined;
  let response: Response;

  if (isDeepSeekAuxiliaryRuntime(auxiliaryRuntime)) {
    response = await fetch(`${context.apiBaseUrl}/chat/completions`, {
      method: 'POST',
      headers: buildHermesRequestHeaders(context),
      body: JSON.stringify(buildStructuredChatCompletionsJsonObjectPayload(context, request)),
      signal: controller?.signal,
    });
  } else {
    response = await fetch(`${context.apiBaseUrl}/responses`, {
      method: 'POST',
      headers: buildHermesRequestHeaders(context),
  ...
  if (!response.ok) {
    ...
    if (!isDeepSeekAuxiliaryRuntime(auxiliaryRuntime) && response.status === 404) {
      response = await fetch(`${context.apiBaseUrl}/chat/completions`, {
        method: 'POST',
        headers: buildHermesRequestHeaders(context),
        body: JSON.stringify(buildStructuredChatCompletionsPayload(context, request)),
        signal: controller?.signal,
      });
    } else {
  ...
  if (!response.ok) {
    const details = await readErrorDetails(response);
    if (isUnsupportedStructuredResponseFormat(response.status, details)) {
      response = await fetch(`${context.apiBaseUrl}/chat/completions`, {
        method: 'POST',
        headers: buildHermesRequestHeaders(context),
        body: JSON.stringify(buildStructuredChatCompletionsPromptOnlyPayload(context, request)),
        signal: controller?.signal,
      });
    } else {
      throw new Error(`Hermes structured output 请求失败: ${response.status}${details ? ` ${details}` : ''}`);
  ...
  const output = (
    ('choices' in data ? extractChatCompletionOutput(data) : extractCompletedOutput(data, ''))
  ).trim();
  if (!output) {
