...
import { requestStructuredData } from './hermes-api.js';
...
function buildWorldStateSideChannelInstructions(input: WorldStateSideChannelInput): string {
  return [
    ...
    '如果当前 turn 看起来与已有 sceneProjection 有关，但对象、位置或状态依赖跨轮补全才能确定，使用 uncertain。',
...
export function createStructuredWorldStateSideChannelGate(): WorldStateSideChannelGate {
  return {
    async decide(input) {
      ...
      const response = await requestStructuredData<StructuredWorldStateSideChannelResponse>({
